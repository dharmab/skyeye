...
func (suite *ControllerTestSuite) SetupSuite() {
	...
	suite.bullseyes = make(chan dcs.Bullseye, 4)

	suite.radar = radar.New(types.CoalitionBlue, suite.bullseyes, suite.updates, suite.fades)
	suite.ctrl = New(suite.radar, types.CoalitionBlue).(*controller)
