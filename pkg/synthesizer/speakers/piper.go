...
func NewPiperSpeaker(v voices.Voice, playbackSpeed float32, playbackPause float32) (Speaker, error) {
	var a asset.Asset
...
func (s *piperSynth) Say(text string) ([]float32, error) {
	synthesized, err := s.tts.Synthesize(text, piper.WithSpeed(s.playbackSpeed), piper.WithPause(s.playbackPause))
	if err != nil {
